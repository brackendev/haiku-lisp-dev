;;; qtui-client.clisp.lisp - CLISP client for qtui-server
;;;
;;; Prerequisites:
...
;;;
;;; CLISP's ext:run-program with :input :stream discards writes to the child on
;;; Haiku, so this script uses ext:make-pipe-io-stream for bidirectional pipes.

(defvar *qtui-stream* nil)

(defun qtui-start ()
  (setf *qtui-stream* (ext:make-pipe-io-stream "./qtui-server")))
...
(defun main ()
  ...
  (format t "~a~%" (qtui-window "main" "Hello from CLISP!" 400 300))
